...
def main():
    ...
    ref_p_name = "CLC_subset_reclass_reprojected.tif"  # linux
    ...
    prediction, prediction_test = random_forest.rf_predict(data, rf_fitted, x_test)
    # geodata.prediction_to_gtiff(prediction, op_name, of_name, path_ref_p, ref_p_name, raster_ext)


    # get accuracy and other metrics
    AccuracyAssessment.accuracy(prediction_test, y_test)  # get overall accuracy
    confusion_matrix = AccuracyAssessment.get_confusion_matrix(y_test, prediction_test)  # get confusion matrix
